...
for i=1+1:X_number-1 
    for j=1+1:Y_number-1
       temp(i,j)=200;
if x_point(i,j)>=16 & x_point(i,j)<=19.2
    if y_point(i,j)>=32 & y_point(i,j)<=35.2
    temp(i,j)=1200;
    tempk(i,j)=1200;
    end
end       
...
for i=NN-1:-1:1
    XX(i+1)=WW(i)-YY(i)*XX(i+2);
            ...
            for i=1:X_number-2
                tempk(i+1,j)=XX(i+1);
    if x_point(i+1,j)>=16 & x_point(i+1,j)<=19.2    
        if y_point(i+1,j)>=32 & y_point(i+1,j)<=35.2      
            tempk(i+1,j)=1200;   
        end
    end                
...
ys2=[35.2 35.2];
ys3=[32 35.2];
line(xs,ys,'color','red');
hold on
line(xs,ys2,'color','red');
hold on
line(xs1,ys3,'color','red');
hold on
line(xs2,ys3,'color','red');
hold on
