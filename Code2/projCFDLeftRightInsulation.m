...
gama=(1+betaa)*ratio+(1+alfaa);
    ...
    for j=1:Y_number-1
    a=abs(x_point(X_number,j)-x_point(X_number-1,j));
    b=abs(y_point(X_number,j)-y_point(X_number-1,j));
    c=abs(sqrt(a^2+b^2)*sin(atan(b/a)-atan((abs(x_point(X_number,j)-x_point(X_number,j+1)))/(abs(y_point(X_number,j)-y_point(X_number,j+1))))));
    a2=abs(x_point(X_number-1,j)-x_point(X_number-1,j+1));
    b2=abs(y_point(X_number-1,j)-y_point(X_number-1,j+1));
    c2=sqrt(a2^2+b2^2);
    d=abs(c2-c);
    temp(1,j)=temp(2,j);
    temp(X_number,j)=(temp(X_number-1,j)*d+temp(X_number-1,j+1)*c)/c2;
    tempk(1,j)=tempk(2,j);
    tempk(X_number,j)=(tempk(X_number-1,j)*d+tempk(X_number-1,j+1)*c)/c2;
    end
