...
gama=(1+betaa)*ratio+(1+alfaa);
            ...
            error=error+abs((tempk(i,j)-temp(i,j))/tempk(i,j));
        end
    end
    for i=1+1:X_number-1 
        for j=1+1:Y_number-1
    ...
    for i=1:X_number-1
    a=abs(x_point(i,Y_number)-x_point(i,Y_number-1));
    b=abs(y_point(i,Y_number)-y_point(i,Y_number-1));
    c=abs(sqrt(a^2+b^2)*sin(atan(b/a)-atan((abs(x_point(i,Y_number)-x_point(i+1,Y_number)))/(abs(y_point(i,Y_number)-y_point(i+1,Y_number))))));
    a2=abs(x_point(i,Y_number-1)-x_point(i+1,Y_number-1));
    b2=abs(y_point(i,Y_number-1)-y_point(i+1,Y_number-1));
    c2=sqrt(a2^2+b2^2);
    d=abs(c2-c);
    aa=abs(x_point(i,1)-x_point(i,2));
    bb=abs(y_point(i,1)-y_point(i,2));
    cc=abs(sqrt(aa^2+bb^2)*sin(atan(bb/aa)-atan((abs(x_point(i,1)-x_point(i+1,1)))/(abs(y_point(i,1)-y_point(i+1,1))))));
    aa2=abs(x_point(i,2)-x_point(i+1,2));
    bb2=abs(y_point(i,2)-y_point(i+1,2));
    cc2=sqrt(aa2^2+bb2^2);
    dd=abs(cc2-cc);
    temp(i,1)=(dd*temp(i,2)+cc*temp(i+1,2))/(cc2);
    temp(i,Y_number)=(d*temp(i,Y_number-1)+c*temp(i+1,Y_number-1))/(c2);
    tempk(i,1)=(dd*tempk(i,2)+cc*tempk(i+1,2))/(cc2);
    tempk(i,Y_number)=(d*tempk(i,Y_number-1)+c*tempk(i+1,Y_number-1))/(c2);
    end
...
dely111=y_point(i,j+1)-y_point(i,j);
delx111=x_point(i,j+1)-x_point(i,j);
dely1=sqrt(delx111^2+dely111^2);ratio=(delx1/dely1)*((delx1+delx2)/(dely1+dely2));
alfaa=delx1/delx2;
betaa=dely1/dely2;
gama=(1+betaa)*ratio+(1+alfaa);
